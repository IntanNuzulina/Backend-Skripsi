    ...
    public function createCharge(Request $request)
    {
        if ($request->has('token')) {
            ...
            $params = [
                'transaction_details' => [
                    'order_id' => rand(),
                    'gross_amount' => $book->flashSale->diskon && $book->flashSale->tanggal_akhir > date('Y-m-d H:i:s') ? $request->amount - $request->amount * ($book->flashSale->diskon / 100) : $request->amount,
                ],
                'customer_details' => [
